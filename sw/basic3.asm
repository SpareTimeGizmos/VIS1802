...
; VIS1802 AND VT1802 MODIFICATIONS
; ---------------------------------
; 18-MAY-24	RLA	Remove R0 and R1 definitions and references.
;			Turn DMAPT, ENINT and DISINT into NOPs.
;
; 20-MAY-20	RLA	Remove the DIS from START.
;			Modify START1 so the initial PC is R3. 
;			Modify START1 to explicitly set the RAM top.  There's
;			no more autosizing of RAM.
;
;   * ENINT and DISINT have been removed.  The VIS1802 firmware uses interrupts
;     to buffer the serial port and keyboard, and to keep the video running.
;     All references to R0 and R1 have been removed.  BASIC is now started
;     with P=R3.
;
;   * DMAPT has been removed.  The VIS1802 hardware doesn't have any DMA
;     devices, and the VT1802 already uses DMA for the video refresh.
; 
;   * The RAM size is fixed and the RAM top is set to protect the tereminal
;     emulator memory at the top of RAM.  RAM is no longer autosized at
;     startup.
; 
;   * WAIT works, but the CPU clock is 2.81MHz.
; 
;   * BYE exits BASIC for a rudimentary monitor that's built in.
; 
;   * INP, OUT, EF, STQ and QST have been removed.  They aren't really useful with a fixed
;     hardware platform.
; 
...
;     the serial port.   Note that it's "SINP" rather than "SIN" to avoid
;     conflict with the trig function by the same name!!!
; 
...
;     I/O; once BASIC is started you can switch I/O with the SINP/SOUT and
;     TIN/TOUT statements.
; 
;   * The RSTIO keyword has been added to restore the console to the default as
;     selected by DIP SW6.  Any error message will automatically reset the 
;     console to the default.
; 
;   * PLOAD, PSAVE, DLOAD and DSAVE use the XMODEM protocol over the serial
;     port.  These commands ALWAYS use the serial port regardless of the current
;     SINP/SOUT and TIN/TOUT selection.
;
...
;[RLA]   On the VIS1802 and VT1802 BASIC fits into the top of  EPROM, after
;[RLA] system firmware.  On both systems, EPROM is mapped from $0000 to
;[RLA] $7FFF, and RAM from $8000 up.  On the VIS1802 the VIS chipset uses
;[RLA] the address space from $F400 to $FFFF, and the firmware uses the two
;[RLA] pages just below that.  On the VT1802 RAM goes all the way to $FFFF,
;[RLA] but the firmware needs a large chunk of that for the frame buffer.
FIRST	EQU	04000H		;[RLA] address (in ROM) of this code
RAM	EQU	08000H		;[RLA] "bottom" of RAM 
RAMTOP	EQU	0EE00H-1	;[RLA] "top" of usable RAM
...
;[RLA] Equates for the VIS1802 firmware!
VIS1802 EQU	0010H		;[RLA] base address for VIS1802 EPROM
VCALL	EQU	VIS1802+(3* 0)	;[RLA]  0 - our standard call routine
VRETURN	EQU	VIS1802+(3* 1)	;[RLA]  1 - our standard return routine
VTRAP	EQU	VIS1802+(3* 2)	;[RLA]  2 - breakpoint trap
CONPUT	EQU	VIS1802+(3* 3)	;[RLA]  3 - send characters to the console
CONGET	EQU	VIS1802+(3* 4)	;[RLA]  4 - read keystrokes from the console
BRKTST	EQU	VIS1802+(3* 5)	;[RLA]  5 - check for console BREAK 
SERPUT	EQU	VIS1802+(3* 6)	;[RLA]  6 - get character from serial port
SERGET	EQU	VIS1802+(3* 7)	;[RLA]  7 - send character to serial port
ISSBRK	EQU	VIS1802+(3* 8)	;[RLA]  8 - test for serial framing error
TIN	EQU	VIS1802+(3* 9)	;[RLA]  9 - redirect console input to PS/2 keyboard
TOUT	EQU	VIS1802+(3*10)	;[RLA] 10 - redirect console output to VIS display
SINP	EQU	VIS1802+(3*11)	;[RLA] 11 - redirect console input to serial port
SOUT	EQU	VIS1802+(3*12)	;[RLA] 12 - redirect console output to serial port
DEFCONS EQU	VIS1802+(3*13)	;[RLA] 13 - restore console to default
BSAVE	EQU	VIS1802+(3*14)	;[RLA] 14 - save BASIC program/data w/XMODEM
BLOAD	EQU	VIS1802+(3*15)	;[RLA] 15 - load BASIC program/data w/XMODEM
BEXIT	EQU	VIS1802+(3*16)	;[RLA] 16 - re-enter VIS monitor
BGMODE	EQU	VIS1802+(3*17)	;[RLA] 17 - select graphics or text mode
BPLOT	EQU	VIS1802+(3*18)	;[RLA] 18 - plot a single point
BLINE	EQU	VIS1802+(3*19)	;[RLA] 19 - draw lines in graphics mode
BPLAY	EQU	VIS1802+(3*20)	;[RLA] 20 - play simple music
BKEY	EQU	VIS1802+(3*21)	;[RLA] 21 - test for key press
BWAIT	EQU	VIS1802+(3*22)	;[RLA] 22 - delay for a number of ticks
BTIME	EQU	VIS1802+(3*23)	;[RLA] 23 - return current system uptime
...
START1	GHI	R2
	...
	SEP	R4
	DW	CLEAR1
;[RLA]   The VIS1802 startup screen/message already acknowledges the RCA BASIC
;[RLA] version and copyright.  Don't repeat it here!
	SEP	R4
	DW	MESOUT
;[RLA]	DB	0AH
;[RLA]	DB	"C RCA 1981"
...
;[RLA]	ANI	03H
;[RLA]	BNZ	A8
;[RLA]	SEP	R4		;[RLA] reset console to the default
;[RLA]	DW	DEFCONS		;[RLA] ...
	NOP
	NOP
	NOP
...
;[RLA]   The KEY function polls the current console input device (be it
;[RLA] the serial port or PS/2 keyboard) for a character.  It takes no
;[RLA] arguments and, because of a BASIC3 quirk, CAN NOT be followed by
;[RLA] an empty set of parenthesis.  So it's
;[RLA]
;[RLA]		LET K = KEY	-> Good
;[RLA]  NOT
;[RLA]		LET K = KEY()	-> Bad!
;[RLA]
;[RLA]   If KEY finds a character waiting it returns the ASCII value,
;[RLA] and if nothing is waiting KEY returns 0.  KEY never blocks or
;[RLA] waits, and it doesn't echo the character read.
KEY:	GHI	R8		;[RLA] save the expression type (integer vs floating)
	STXD			;[RLA]  ... for the jump to FCONV at the end
	INC	R9		;[RLA] advance the computation stack
	INC	R9		;[RLA] ...
	INC	R9		;[RLA] ...
	INC	R9		;[RLA] ...
;;	SEP	R4		;[RLA] switch to the firmware SCRT routines
;;	 DW	 MSCRT		;[RLA] ...
	SEP	R4		;[RLA] and call the BASIC KEY function
	 DW	 BKEY		;[RLA] ...
;;	SEP	R4		;[RLA] restore BASIC's SCRT routines
;;	 DW	 RSCRT		;[RLA] ...
	SEP	R4		;[RLA] reset BASIC argument stack
	 DW	 ZRST		;[RLA] ...
	INC	R9		;[RLA] ...
	GLO	R8		;[RLA] store the result on the stack
	STR	R9		;[RLA] ...
	DEC	R9		;[RLA] skip the other three bytes
	DEC	R9
	DEC	R9
	LBR	FCONV		;[RLA] convert result to floating
...
;	DB	064H, 'S', 'T', 80H+'Q', 095H				;[RLA] was ..STQ
...
;	DB	067H, 'D', 'I', 'S', 'I', 'N', 80H+'T', 0A3H		;[RLA] was ..DISINT
	...
	DB	065H, 'S', 'I', 'N', 80H+'P', 0A9H			;[RLA] ..SINP (was DIN)
	DB	065H, 'S', 'O', 'U', 80H+'T', 0AAH			;[RLA] ..SOUT (was DOUT)
;	DB	064H, 'O', 'U', 80H+'T', 0ABH				;[RLA] was ..OUT
	DB	066H, 'R', 'S', 'T', 'I', 80H+'O', 0ACH			;[RLA] ..RSTIO (was CLOSE)
	DB	064H, 'B', 'Y', 80H+'E', 0ADH				;..BYE
	DB	065H, 'E', 'X', 'I', 80H+'T', 0AEH			;..EXIT
	DB	065H, 'L', 'I', 'N', 80H+'E', 0AFH			;[RLA] LINE was ..WFLN
	DB	065H, 'P', 'L', 'O', 80H+'T', 0B0H			;[RLA] PLOT was ..RFLN
...
;	 ..B8 IS USED FOR \ TO CLOSE BINARY DATA
	...
	DS	7+2-1+5-2+5+5-3	;[RLA] adjust for the keywords removed
				;[RLA]  ... -1 for SIN -> SINP 
				;[RLA]  ... -3 for GMODE -> GRAPHICS
				;[RLA]  ... -1 FOR QST -> TIME
				;[RLA] PLOAD is at $4324 ...
...
;[RLA]   The TIME function returns the time since the system was last
;[RLA] reset, in 60HZ VRTC "ticks".  It takes no arguments and, becuase
;[RLA] of the way BASIC3 works, CAN NOT be followed by an empty set of
;[RLA] parenthesis.  For example
;[RLA]
;[RLA]		10 DEFINT A
;[RLA]		20 LET A = TIME
;[RLA] 		...
;[RLA]
;[RLA]   Note that this function explicitly returns a 32 bit INTEGER and
;[RLA] NOT a floating point value.  You can always convert it to floating
;[RLA] point with FNUM(), however this will lose important bits from the
;[RLA] low end.
;[RLA]
;[RLA]   FWIW, just in case you're wondering, a 32 bit integer incremented
;[RLA] at 60Hz will roll over after a little over 800 days...
TIME:	GHI	R8		;[RLA] set the return type to INTEGER
	ORI	8		;[RLA] ...
	STXD			;[RLA] ... for the branch to FCONV, later
	INC	R9		;[RLA] advance the computation stack
	INC	R9		;[RLA] ...
	INC	R9		;[RLA] ...
	INC	R9		;[RLA] ...
;;	SEP	R4		;[RLA] switch to the firmware SCRT routines
;;	 DW	 MSCRT		;[RLA] ...
	SEP	R4		;[RLA] and get the current system uptime in RA:R8
	 DW	 BTIME		;[RLA] ...
;;	SEP	R4		;[RLA] restore BASIC's SCRT routines
;;	 DW	 RSCRT		;[RLA] ...
	SEP	R4		;[RLA] reset BASIC argument stack
	 DW	 ZRST		;[RLA] ...
	INC	R9		;[RLA] store the 32 bit result on the BASIC stack
	GLO	R8		;[RLA] ...
	STR	R9		;[RLA] ...
	DEC	R9		;[RLA] ...
	GHI	R8		;[RLA] ...
...
;[RLA] IOROUT is at $4500
	DS	0BH

;......................................................
;	..I/O ROUTINES FOR VT/VIS1802
;	..IOROUT.D
;	..........
;[RLA]   This was totally rewritten (and most of this code was removed)
;[RLA] to work with the VIS/VT1802 firmware.  That code takes care of
;[RLA] saving and restoring all registers used already, so no need to
;[RLA] worry about that here.  And it conveniently uses the same SCRT
;[RLA] routines as BASIC, so no worries there either.
IOROUT	SEP R5			;[RLA] I/O initialization - ignored
	DS	2		;[RLA] each entry is 3 bytes!
BREAKT	LBR	BREAKC		;[RLA] test for break
OUTC	LBR	OUTPT2		;[RLA] output the character from RF.1
INPDC	SEP R4			;[RLA] read a character from the console
	DW	CONGET		;[RLA] ...
	BR	INPDE		;[RLA] return it in D and RF.1
OUTPT2	GHI	RF		;[RLA] output the character from RF.1
	SEP	R4		;[RLA] ...
	DW	CONPUT		;[RLA] ...
INPDE	GHI	RF		;[RLA] always return the same character
	SEP	R5		;[RLA] and we're done here
BREAKC	SEP	R4		;[RLA] test for console break
	DW	BRKTST		;[RLA] ...
	SEP	R5		;[RLA] return DF=1 if break found

;[RLA] PSAVE IS AT $453F ...
	DS	27H
...
START	LDI	HIGH STCALL
	...
	PHI	R8		;R8 = XXFFH
	SEX	R2		;X = R2
;[RLA]   All the code which used to size RAM has been removed.  The
;[RLA} RAM size in the VT/VIS1802 is fixed, and more importantly,
;[RLA} we have to be supercareful not to overwrite the part used
;[RLA] by the video firmware.  That's always stored at the end.
	LDI	HIGH RAMTOP	;[RLA] RAM ends at 0xEDFF
	PHI	R2		;[RLA] ...
	LBR	START1		;[RLA] and go finish the startup

;[RLA] STKEY needs to be at $4696
	DS	0EH		;[RLA] ...
...
;[RLA]   The PLOT statement plots a single point on the VIS display in
;[RLA] graphics mode.  It takes two arguments, the X and Y coordinates
;[RLA] of the point to be plotted ...
;[RLA]
;[RLA]		PLOT(X,Y)
;[RLA]
;[RLA]   Note that the VIS coordinate space is 240x192, so X and Y must
;[RLA] be positive integers .LE. 239 (for X) and .LE. 191 (for Y).
PLOT:	SEP	R4		;[RLA] evaluate the arguments
	DW	EVIARG		;[RLA] ...
	XRI	2		;[RLA] we should find exactly two arguments
	BZ	PLOT1		;[RLA] yes - good
	SEP	R4		;[RLA] no - error
	 DW	 ERROR		;[RLA] ...
	 DB	 91		;[RLA] wrong number of arguments for PLOT
PLOT1:	SEP	R4		;[RLA] fetch (X,Y) and store in P1
	DW	GETXY		;[RLA] ...
;;	SEP	R4		;[RLA] switch to the firmware SCRT routines
;;	DW	MSCRT		;[RLA] ...
	SEP	R4		;[RLA] and call the BASIC PLOT function
	DW	BPLOT		;[RLA] ...
PLOT2:;;	SEP	R4		;[RLA] restore SCRT and DON'T CHANGE DF!!
;;	DW	RSCRT		;[RLA] ...
	LBNF	PLOT3		;[RLA] if DF=1 the coordinates were invalid
	SEP	R4		;[RLA] so give an error message
	 DW	 ERROR		;[RLA] ...
	 DB	 93		;[RLA] INVALID coordinates for PLOT() or LINE()
PLOT3:	SEP	R5		;[RLA] all done

;[RLA]   The LINE statement draws a line between two points on the VIS
;[RLA] display.  This actually needs four arguments, (X0,Y0) for the
;[RLA] first endpoint, and the (X1,Y1) for the second endpoint.
;[RLA]
;[RLA]		LINE(X0,Y0,X1,Y1)
;[RLA]
;[RLA]   As with PLOT(), if any of the coordinates would be out of range
;[RLA] then error 93 occurs.
LINE:	SEP	R4		;[RLA] evaluate the arguments
	DW	EVIARG		;[RLA] ...
	XRI	4		;[RLA] this time there should be four
	BZ	LINE1		;[RLA] yes - good
	SEP	R4		;[RLA] no - error
	 DW	 ERROR		;[RLA] ...
	 DB	 92		;[RLA] wrong number of arguments for LINE
LINE1:	SEP	R4		;[RLA] load the first set of coordinates
	DW	GETXY		;[RLA]  ... into RE
	GLO	RE		;[RLA] then transfer RE to RA
	PLO	RA		;[RLA] ...
	GHI	RE		;[RLA] ...
	PHI	RA		;[RLA] ...
	SEP	R4		;[RLA] then do it again for the second
	DW	GETXY		;[RLA]  ... pair of coordinates
;;	SEP	R4		;[RLA] switch to the firmware SCRT routines
;;	DW	MSCRT		;[RLA] ...
	SEP	R4		;[RLA] and call the BASIC LINE function
	DW	BLINE		;[RLA] ...
	LBR	PLOT2		;[RLA] let the PLOT code handle the rest

;[RLA]   This routine is used by both PLOT and LINE to get an (X,Y)
;[RLA] coordinate pair from the BASIC argument stack and store the
;[RLA] values in P1.  We're only interested in the integer values from
;[RLA] the BASIC argument stack, and in fact we only use the lowest
;[RLA] byte (8 bits) of that.  We really should check that the upper
;[RLA] 24 bits of each argument is zero, but I'm lazy ...
GETXY:	INC	R9		;[RLA] skip over 3 MSBs for Y
	INC	R9		;[RLA] ...
	INC	R9		;[RLA] ...
	LDN	R9		;[RLA] and get the LSB of the Y argument
	PLO	RE		;[RLA] store that in P1.0 for the VIS firmware
	GLO	R9		;[RLA] then fix up the argument stack
	SMI	7		;[RLA] ...
	PLO	R9		;[RLA] ...
	INC	R9		;[RLA] and do it all again to get X
	INC	R9		;[RLA] ...
	INC	R9		;[RLA] ...
	LDN	R9		;[RLA] get the LSB of the X argument
	PHI	RE		;[RLA] store that in P1.1 for the VIS firmware
	GLO	R9		;[RLA] finally fix up the argument stack
	SMI	7		;[RLA] ...
	PLO	R9		;[RLA] ...
	SEP	R5		;[RLA] and we're done

;[RLA]   The PLAY statement plays a tune using the sound generator in the
;[RLA] VIS chipset.  The argument to PLAY is a string of characters which
;[RLA] specify the notes, durations, octaves, volume, etc of the notes to
;[RLA] be played according to rules that are much too complicated to go into
;[RLA] here!  The firmware listing gives a complete description of the
;[RLA] syntax for the music string.
;[RLA]
;[RLA]   Syntactically BASIC treats PLAY like a REM statement and just
;[RLA] completely ignores the remainder of the line.  Spaces are ignored,
;[RLA] but otherwise the remainder of the line will be "played", and in
;[RLA] particular, quotation marks around the string are NOT required 
;[RLA] (and should NOT be used).
;[RLA]
;[RLA]   This is probably not the best way to handle it, since it means
;[RLA] that BASIC string variables and functions cannot be used with PLAY,
;[RLA] but it's the best I know how to do!
PLAY:;;	SEP	R4		;[RLA] switch to the firmware SCRT routines
;;	DW	MSCRT		;[RLA] ...
	SEP	R4		;[RLA] and call the BASIC PLAY function
	DW	BPLAY		;[RLA] ...
;;	SEP	R4		;[RLA] restore BASIC's SCRT routines
;;	DW	RSCRT		;[RLA] ...
PLAY1:	LDN	RB		;[RLA] get the character we can't play
	XRI	0DH		;[RLA] is it <CR> ?
	LSNZ			;[RLA] ???
	 SEP	 R5		;[RLA] yes - all is good
	 NOP			;[RLA] ...
	SEP	R4		;[RLA] otherwise flag an error
	 DW	 ERROR		;[RLA] ...
	 DB	 94		;[RLA] INVALID CHARACTER IN PLAY STRING

;[RLA] RSTIO is at $474A
	DS	24H+6+6+3
...
;	..BYE.D
;	.......
;[RLA]   On the VT/IS1802 the BYE command exits to the simple
;[RLA] monitor that's built into the firmware.
BYE	SEP	RD
	DB	0A9H
	PHI	RE
	LBR	BEXIT	;[RLA] re-enter VIS monitor
;......................................................
;	..APPENDAGE TO LIS..
;	..AH5.B
;	....................
...
EXTAB	DW	REM		; 080H
	...
	DW	DSAVE		; 0A0H
	DW	DLOAD		; 0A1H
	DW	PLAY		; [RLA] 0A2H was ENINT
	DW	NOOP		; [RLA] 0A3H was DISINT
	DW	GMODE		; [RLA] 0A4H was DMAPT
	DW	TIN		; 0A5H
	DW	TOUT		; 0A6H
	DW	TRACE		; 0A7H
	DW	USCALL		; 0A8H
	DW	SINP		; [RLA] 0A9H was DIN
	DW	SOUT		; [RLA] 0AAH was DOUT
	DW	NOOP		; [RLA] 0ABH was OUT
	DW	DEFCONS		; [RLA] 0ACH was CLOSE
	DW	BYE		; 0ADH
	DW	EXIT		; 0AEH
	DW	LINE		; 0AFH LINE [RLA] was SUWBF
	DW	PLOT		; 0B0H PLOT [RLA] was SURBF
	DW	RENUM		; 0B1H
	DW	EDIT		; 0B2H
	DW	FORMAT		; 0B3H
...
;	.................................
	...
	DW	SGN		; 0E7H
	DW	MODU		; 0E8H
	DW	KEY		; [RLA] 0E9H was INP
	DW	NOTB		; 0EAH
	DW	PI		; 0EBH
	DW	FVAL		; 0ECH
	DW	ERROR		; 0EDH
	DW	MEM		; 0EEH
	DW	TIME		; [RLA] 0EFH was QST
	DW	NOOP		; [RLA] 0F0H was EF
...
;......................................................
;	..MOVE STANDARD CALL AND RETURN POINTERS TO VIS/VT1802 STANDARD ...
;	..MSCRT.B
;	..........................................
MSCRT	LDI	HIGH VCALL	;[RLA]
	PHI	R4		;[RLA]
	PHI	R5		;[RLA]
	LDI	LOW VCALL	;[RLA]
	PLO	R4		;[RLA]
...
;[RLA]   The GRAPHICS statement switches between graphics and text
;[RLA] modes and, as a side effect, can also clear graphics memory.
;[RLA]
;[RLA]		GRAPHICS 0 -> switch to text display
;[RLA]		GRAPHICS 1 -> switch to graphics display
;[RLA]		GRAPHICS 3 -> switch to graphics display AND clear it
;[RLA]
;[RLA] FWIW, the CLS command works to clear the text mode display.
GMODE:	SEP	R4		;[RLA] evaluate the argument
	DW	GVAL		;[RLA] ...
;;	SEP	R4		;[RLA] switch to the firmware SCRT routines
;;	DW	MSCRT		;[RLA] ...
	SEP	R4		;[RLA] and call the BASIC GMODE function
	DW	BGMODE		;[RLA] ...
;;	LBR	RSCRT		;[RLA] restore the SCRT pointers and we're done
	SEP	R5

;[RLA] CLARS is at $4FD0
	DS	21H+5
...
;	..PAUSE.A
;	.................
PAUSE	SEP	R4		;[RLA] get the argument
	DW	GVAL		;[RLA] ...
;;	SEP	R4		;[RLA] switch to the firmware SCRT
;;	DW	MSCRT		;[RLA] ...
	SEP	R4		;[RLA] and then wait as needed
	DW	BWAIT		;[RLA] ...
;;	LBR	RSCRT		;[RLA] restore the SCRT routines and we're done
	SEP	R5

;[RLA] PCHARS is at $53F2
	DS	0CH+5
...
RSTFLG	GHI	R8
	...
	LDI	LOW AU3
	PLO	RA
	NOP		;; [RLA]	LDI	HIGH INTPT
	NOP		;; [RLA]
	NOP		;; [RLA]	PHI	R1
	NOP		;; [RLA]	LDI	LOW INTPT
	NOP		;; [RLA]
	NOP		;; [RLA]	PLO	R1
	LDI	HIGH WP1
	PHI	RF
